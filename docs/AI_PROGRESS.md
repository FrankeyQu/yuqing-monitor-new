...
- **审计目标**：按用户确认的后续治理路径，先生成“疑似误预警候选清单”口径，仅做线上只读统计和抽样，不直接修改生产数据库。
- **线上只读结果**：`campus_monitor_result` 当前 `result_status=alerted` 且关联 `alert_id` 的监测命中共 `473` 条，全部来自 `新疆大学` 任务的 `alert_mode=all_hits` 历史口径，且对应预警单均为 `pending`。
- **候选口径**：待处理预警、任务为 `all_hits`、无 `matched_negative_words`、监测情感为 `neutral/positive/none`、监测风险为 `normal/concern`、原始接入风险为 `normal` 或空、原始情感不是负面。按该口径命中 `453` 条疑似“普通关键词命中被生成预警”的候选。
- **候选拆分**：`451` 条未关联线索，适合进入后续“批量取消预警”候选池；`2` 条已关联线索，需单独复核，避免影响已经进入线索链路的数据。另有 `20` 条因存在负面词、负面情感或更高风险等级，被排除在候选之外，应继续保留为待处理预警。
- **抽样判断**：样本标题包括“新疆大学了解/报考参考”“新大研一布置小窝”“新疆大学博达校区7号楼”“新大夜景”“新疆大学博士宿舍”等，主要是学校普通提及、校园生活、招生报考、房产/周边内容；当前看更像普通监测命中，不应直接作为预警处置。

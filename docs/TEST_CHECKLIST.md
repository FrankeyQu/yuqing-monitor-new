...
- 2026-05-18：舆情态势图表数据源与运行任务底栏调整验证；`campus-web npm run build` 通过，仅保留既有 Rollup PURE 注释和 chunk 体积警告；`git diff --check` 通过（仅 CRLF 提示）；Playwright 本地 mock 数据打开 `/` 与 `/?mode=screen`，确认媒体来源坐标中文化、运行任务底栏展示关键词/AI分析/调度/近次命中。

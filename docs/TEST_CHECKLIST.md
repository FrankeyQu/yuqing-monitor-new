...
.\mvnw.cmd "-DskipTests=false" "-Dmaven.test.skip=false" test    # 启用后端测试，当前至少 68 个用例通过
```

⚠️ **当前状态**：
- `skipTests=true` 在 pom.xml 中硬编码
- 测试目录 `src/test/java/` 当前有 20 个 Java 测试类
- 接入模块、ID 序列化、报告 AI 快照和监测 AI 标题正文比较测试共至少 68 个用例
...
- [ ] 客户侧监测信息页不展示“合并相似信息”和“匹配对象”筛选；后端 `similarDedup/matchScope` 参数仅作为兼容能力保留。
- [ ] “新增人工线索”保存后进入线索库，不新增普通监测命中；“添加文章”入口不可见。
- [ ] 批量操作对监测命中支持转线索、转预警、取消预警；对已转线索支持研判、加入事件；混选时展示成功、失败、跳过数量。
...
### 2026-05-18 监测疑似误预警治理工具验证
- 线上只读审计：当前 `result_status=alerted` 且有关联预警的监测命中共 `473` 条，其中 `453` 条符合“历史 all_hits + 待处理预警 + 无负面证据 + 原始风险普通”疑似误预警候选；`451` 条未关联线索，`2` 条已关联线索，`20` 条有负面证据应保留。
- 最终 SQL 复核：按 Mapper 候选条件只读查询，未关联线索可处理候选 `451` 条、总候选 `453` 条、负面证据保留预警 `20` 条。
- 后端验证：使用 JDK8 执行 `.\mvnw.cmd -DskipTests compile`、`.\mvnw.cmd -DskipTests package` 通过。
- 前端构建：`campus-web npm run build` 通过，仅保留既有 Rollup PURE 注释和 chunk 体积警告。
- 验收口径：`/campus/monitor/result/alert-cleanup/preview` 只读返回候选汇总和样本；`/campus/monitor/result/alert-cleanup/execute` 需要 `confirmText=确认取消预警`，后端重新筛选候选，不接受前端提交 ID 列表，默认跳过已关联线索。
- 前端验收口径：`/monitor` 工具栏显示“误预警治理”，无监测操作权限时禁用；弹窗展示疑似误预警、可直接取消、已关联线索、保留预警四类数字，执行前二次确认，执行后刷新列表、预警表和候选统计。

### 2026-05-18 监测命中 AI 分析标题正文比较验证
...
### 2026-05-18 监测风险等级口径收紧验证
- 后端编译：使用 JDK8 执行 `.\mvnw.cmd -DskipTests compile` 通过。
- 前端构建：`campus-web npm run build` 通过，仅保留既有 Rollup PURE 注释和 chunk 体积警告。
- 验收口径：普通关键词命中、无负面词/风险词、原始风险普通、情感中性的新增监测结果应为 `riskLevel=normal/riskScore=0`，不因关键词数量进入“一般预警”。
- 验收口径：命中负面词/风险词、原始风险非普通或负面情感的新增监测结果应至少为 `riskLevel=concern/riskScore>=45`。
- 验收口径：手动 AI 分析返回 `riskLevel=concern` 或 `sentiment=negative` 时，应同步更新监测结果风险等级；已转未归档线索同步风险等级；不自动改变 `resultStatus`，不自动创建预警。
...
- 数据库现状：Flyway `1.44 CampusReportPromptAndTemplates`、`1.45 CampusMonitorAiAnalysis`、`1.46 CampusDashboardScreenUnification` 均 success=1；AI 功能绑定和提示词已存在，DeepSeek 凭据引用已配置。
- 接入观察：`2026-05-17 23:42:00` 后接入运行失败数为 0，说明软删除去重修复后的重复键问题未继续出现。
...
- 线上发布：备份目录 `/home/ubuntu/yuqing-backups/deploy-20260517-234447-ingest-deleted-dedup`；已覆盖后端 jar 并重启，`yuqing` active 且监听 `8084`。
- 线上验收：触发问题任务 `2054608482077904896` 调度后，最新 run `2056039386365169664` 为 `success`，`fetched_count=20`、`duplicate_count=20`、`fail_count=0`；任务失败计数清零，`last_error_type=NULL`。
...
- 2026-05-18：监测信息预警状态文案澄清验证；`campus-web npm run build` 通过，仅保留既有 Rollup PURE 注释和 chunk 体积警告；`git diff --check` 通过（仅 CRLF 提示）；检查 `/monitor` 中处理状态筛选和状态列不再展示“已预警”，改为“已生成预警”；风险等级在监测信息页展示为“普通关注 / 一般风险 / 重大风险 / 特别重大风险”；悬停状态说明能够解释已生成预警单与风险命中的区别；已发布前端到服务器，备份目录 `/home/ubuntu/yuqing-backups/deploy-20260518-041020-monitor-alert-wording`，`yuqing/nginx/mariadb/redis-server` active，`/`、`/monitor`、`/situation` 返回 200；本轮不执行历史数据清理。

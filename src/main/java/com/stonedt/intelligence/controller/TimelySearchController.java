...
//
	...
	public String getSpiderData(String keyword,String website_id,String pageNoData) {
		if (legacySpiderOpen == null || legacySpiderOpen != 1) {
			return disabledRealtimeSearchResponse("旧实时检索桥接默认关闭，请完成试运行接入确认后再开启");
		}
		if (StringUtils.isBlank(legacySpiderWebsocketUrl) || StringUtils.isBlank(xmlPath)) {
			return disabledRealtimeSearchResponse("旧实时检索桥接缺少已确认的本地配置");
		}
...
//		if(!"1".equals(website_id)) {
			...
			String data = WebSocketUtils.data(keyword, xml_data, pageNoData, legacySpiderWebsocketUrl);
...
//		}
	...
	private String disabledRealtimeSearchResponse(String message) {
		JSONObject jsonObject = new JSONObject();

...
public class UserController {
   ...
   /* @SystemControllerLog(module = "系统设置", submodule = "系统设置-账号管理", type = "查询", operation = "detail")*/
     ...
     * @return
     */
    @SystemControllerLog(module = "系统设置", submodule = "系统设置-账号管理", type = "设置新密码", operation = "setPassword")
    @PostMapping(value = "/setPassword")
    @ResponseBody
    public ResultUtil setPassword(@RequestParam("newPassword") String newPassword,
                                   @RequestParam(value = "confirmPassword", required = false) String confirmPassword,
                                   HttpServletRequest request) {
        if (newPassword == null || newPassword.trim().isEmpty()) {
            return ResultUtil.build(203, "新密码不能为空", null);
        }
        if (confirmPassword != null && !confirmPassword.isEmpty() && !newPassword.equals(confirmPassword)) {
            return ResultUtil.build(203, "两次输入的新密码不一致", null);
        }
        User user = userUtil.getuser(request);
        int updated = userService.updatePassword(user.getUser_id(), MD5Util.getMD5(newPassword));
        if (updated > 0) {
            return ResultUtil.build(200, "设置成功", null);
        }
        return ResultUtil.build(203, "设置失败", null);
    }

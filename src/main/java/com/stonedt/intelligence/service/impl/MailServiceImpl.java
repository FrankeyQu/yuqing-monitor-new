...
public class MailServiceImpl implements MailService {
    ...
    private static final String PRODUCT_NAME = "校园舆情综合研判平台";
    ...
    public void sendWarningMail(MailConfig mailConfig, String content) throws MessagingException, GeneralSecurityException, UnsupportedEncodingException {
        sendMail(mailConfig, PRODUCT_NAME + "  预警邮件推送", content, PRODUCT_NAME, true);
    }
    ...
    public void sendWarningMail(MailConfig mailConfig, byte[] byteArray) throws MessagingException, GeneralSecurityException, UnsupportedEncodingException {
        ...
        MimeMessage message = new MimeMessage(session);
        // 指明邮件的发件人
        message.setFrom(new InternetAddress(mailConfig.getUsername(), PRODUCT_NAME));
        // 指明邮件的收件人，现在发件人和收件人是一样的，那就是自己给自己发
        message.setRecipient(Message.RecipientType.TO, new InternetAddress(mailConfig.getTo()));
        // 邮件的标题
        Date date = new Date();
        String format = sdf.format(date);
        String title = PRODUCT_NAME + format;
    ...
    public ResultUtil saveMailConfig(MailConfig mailConfig, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        log.info("发送测试邮件");
        try {
            sendMail(mailConfig, PRODUCT_NAME + "  邮件配置测试", "邮件配置测试", PRODUCT_NAME, true);
        } catch (GeneralSecurityException | UnsupportedEncodingException | MessagingException e) {
            log.error("邮件发送失败",e);
